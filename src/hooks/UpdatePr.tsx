...
export const UpdatePr: React.FC<UpdatePrProps> = ({ pr }) => {
  ...
  const [bYReview, setBYReview] = useState<boolean>(
    reviewBooleanReturner(bYStatus)
  );
  const [aHReview, setAHReview] = useState<boolean>(
    reviewBooleanReturner(aHStatus)
  );
  const [hTReview, setHTReview] = useState<boolean>(
    reviewBooleanReturner(hTStatus)
  );
  ...
  const submitUpdate: SubmitUpdate = (selectedPr) => {
    ...
    if (platform !== "") {
      selectedPr.Platform = platform;
    }
    if (version !== "") {
      selectedPr.Version = version;
    }
    if (comment !== "") {
      selectedPr.Comment = comment;
    }
    if (link !== "") {
      selectedPr.PR = link;
    }
    if (size !== "") {
      selectedPr.Size = size;
    }
    if (difficulty !== "") {
      selectedPr.Difficulty = difficulty;
    }
    if (status !== "") {
      selectedPr.Status = status;
    }
    ...
    <div className="update-main-div">
      ...
      <div className="update-section">
        <UpdatePrForm4
          ...
          aHReview={aHReview}
          hTReview={hTReview}
          setBYReview={setBYReview}
          setAHReview={setAHReview}
          setHTReview={setHTReview}
        />
      </div>
    </div>
  );
